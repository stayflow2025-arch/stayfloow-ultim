...
import { Star, Heart, MapPin, ChevronRight, ShieldCheck } from "lucide-react";
...
export function CarResultCard({ car, viewMode = "list" }: CarResultCardProps) {
  ...
  const detailUrl = `/cars/${car.id}${currentParams ? `?${currentParams}` : ""}`;
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="mt-2 space-y-3">
          <p className="text-[#6B6B6B] text-[14px] leading-relaxed line-clamp-2">
            {car.description}
            <Link href={detailUrl} className="text-[#10B981] hover:underline ml-1 font-medium">En savoir plus</Link>
          </p>

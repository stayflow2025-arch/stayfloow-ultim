      ...
      <div className="bg-white/80 backdrop-blur-md border-b sticky top-16 z-40 py-3 px-6">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <Button variant="ghost" onClick={() => router.back()} className="font-bold text-slate-600 hover:text-primary transition-colors">
            <ChevronLeft className="mr-2 h-4 w-4" /> {t('back') || 'Retour'}
          </Button>
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="space-y-2">
              <div className="flex flex-wrap gap-2 mb-2">
                <Badge variant="secondary" className="bg-primary/10 text-primary border-primary/20 font-black uppercase">{t('elite') || 'STAYFLOOW SELECTION'}</Badge>
                <Badge variant="outline" className="text-amber-600 border-amber-200 bg-amber-50">{t('rating_label') || 'Note Client'} : 9.8</Badge>
              </div>
            ...
            <div className="rounded-3xl overflow-hidden shadow-2xl bg-slate-200 aspect-video relative group border-4 border-white">
              ...
              <div className="absolute top-6 right-6 bg-black/50 text-white px-4 py-2 rounded-full text-xs font-black backdrop-blur-md">
                {displayData.photos?.length || 1} {t('photos') || 'PHOTOS'}
              </div>
            </div>
            ...
            <Card className="border-none shadow-xl rounded-3xl overflow-hidden bg-white">
              <CardContent className="p-8 md:p-10 space-y-6">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-2xl font-black text-slate-900">{t('presentation') || 'Présentation'}</h2>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
                  <div className="space-y-4">
                    <h3 className="font-black text-slate-900 flex items-center gap-2">
                      <CheckCircle className="h-5 w-5 text-primary" /> {t('highlights') || 'Points forts'}
                    </h3>
                    <ul className="space-y-3 text-sm text-slate-500 font-medium">
                      <li>• {t('prime_location') || 'Emplacement stratégique'}</li>
                      <li>• {t('dedicated_staff') || 'Personnel multilingue dédié 24h/24'}</li>
                      <li>• {t('local_gastronomy') || 'Restauration gastronomique locale sur place'}</li>
                    </ul>
                  </div>
                  <div className="space-y-4">
                    <h3 className="font-black text-slate-900 flex items-center gap-2">
                      <ShieldCheck className="h-5 w-5 text-primary" /> {t('stayfloow_security') || 'Sécurité StayFloow'}
                    </h3>
                    <p className="text-sm text-slate-500 font-medium leading-relaxed">
                      {t('security_desc') || 'Cet établissement respecte scrupuleusement notre charte de qualité "Gold Partner". Votre réservation est protégée.'}
                    </p>
                  </div>
                </div>
              </CardContent>
            ...
            <div className="space-y-6">
              <h2 className="text-2xl font-black text-slate-900 flex items-center gap-3">
                <Star className="h-6 w-6 text-primary" /> {t('amenities_label') || 'Ce que propose cet établissement'}
              </h2>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {displayData.details?.amenities?.map((amenity: string) => (
                  <div key={amenity} className="flex items-center gap-3 p-5 bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md hover:border-primary/20 transition-all group">
                    ...
                    <span className="text-sm font-bold text-slate-700">{t(amenity) || amenity}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="lg:col-span-1">
              ...
              <CardContent className="p-0">
                <div className="bg-primary p-8 text-white relative overflow-hidden">
                  <div className="relative z-10">
                    <p className="text-sm font-bold opacity-80 mb-1 uppercase tracking-widest">{t('exclusive_offer') || 'Offre Exclusive'}</p>
                    <div className="flex justify-between items-baseline">
                      <h3 className="text-4xl font-black">{formatPrice(displayData.price)}</h3>
                      <p className="text-sm font-bold opacity-80">/ {t('per_night') || 'nuit'}</p>
                    </div>
                  </div>
                ...
                <div className="p-8 space-y-8 bg-white">
                  <div className="space-y-4">
                    <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex items-center justify-between cursor-pointer hover:bg-slate-100 transition-colors group">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">{t('dates') || 'Dates'}</p>
                          <p className="font-bold text-sm">{t('choose_dates') || 'Choisir mes dates'}</p>
                        </div>
                      </div>
                    ...
                    <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex items-center justify-between cursor-pointer hover:bg-slate-100 transition-colors group">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">{t('travelers_label') || 'Voyageurs'}</p>
                          <p className="font-bold text-sm">2 {t('adults') || 'adultes'}, 0 {t('children') || 'enfant'}</p>
                        </div>
                      </div>
                      <ChevronRight className="h-4 w-4 text-slate-300" />
                    </div>
                  ...
                  <Button className="w-full h-16 text-xl font-black bg-primary hover:bg-primary/90 shadow-xl shadow-primary/20 rounded-2xl active:scale-95 transition-all" asChild>
                    <Link href={`/properties/${propertyId}/book`}>
                      {t('reserve_now') || 'Réserver maintenant'}
                    </Link>
                  </Button>

                  <div className="space-y-4 pt-4">
                    <div className="flex justify-between text-sm font-medium">
                      <span className="text-slate-500 underline cursor-help decoration-dotted">{formatPrice(displayData.price)} x 3 {t('nights') || 'nuits'}</span>
                      <span className="font-bold text-slate-900">{formatPrice(displayData.price * 3)}</span>
                    </div>
                    <div className="flex justify-between text-sm font-medium">
                      <span className="text-slate-500 underline cursor-help decoration-dotted">{t('service_fees') || 'Frais de service StayFloow'}</span>
                      <span className="text-green-600 font-bold">{t('free') || 'OFFERT'}</span>
                    </div>
                  ...
                  <div className="bg-slate-50 p-4 rounded-xl flex items-center gap-3 border border-dashed border-slate-200">
                    ...
                    <p className="text-[10px] text-slate-500 font-bold leading-tight uppercase">
                      {t('secure_payment_desc') || 'Paiement 100% sécurisé via StayFloow Pay'}
                    </p>
                  </div>
                </div>
              </CardContent>
            ...
            <Link href="/contact" className="mt-8 block p-6 bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-all">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h4 className="font-bold text-sm text-slate-900">{t('need_help') || "Besoin d'aide ?"}</h4>
                  <p className="text-xs text-slate-500">{t('contact_host_desc') || "Contactez l'hôte ou notre support 24/7."}</p>
                </div>
              </div>
            </Link>
          </div>
